...
extension MySQLUint8ListExtension on Uint8List {
  ...
  // Dans tes extensions de Uint8List
  Tuple2<Uint8List, int> getBinaryLengthEncoded(int startOffset) {
    final tmp = Uint8List.sublistView(this, startOffset);
    final bd = ByteData.sublistView(tmp);

    // 1. On récupère la longueur encodée (item1 = valeur, item2 = taille de l'entier)
    final strLength = bd.getVariableEncInt(0);
    final int dataLen = strLength.item1.toInt();
    final int offsetToData = strLength.item2;

    // 2. On extrait les octets bruts SANS utf8.decode
    final rawData = Uint8List.sublistView(
      tmp,
      offsetToData,
      offsetToData + dataLen,
    );

    return Tuple2(rawData, offsetToData + dataLen);
  }
}
